...
# -------------------------------------------------------------------
# Streamlit UI for open-field growth simulations (reproducible & aligned)
# - Deterministic RNG: per-(year,sim) seeds for both Python and Numba paths
# - Yearly arrays sorted chronologically (stateful soil moisture)
# - Kernel uses same season timing as scalar (planting_duration-based)
# - Default float64 (matches baseline); optional float32 toggle
# - Optional daily-series capture for a few sims/year
# -------------------------------------------------------------------
...
st.title("🍓 Open-field Growth Simulation — Reproducible")
...
@st.cache_data(show_spinner=False)
def preprocess_year_arrays(df: pd.DataFrame, use_float32: bool):
    """
    Prepare dense arrays per year for the kernel, in chronological order.
    """
    out = {}
    ...
    for y, b in df2.groupby("Year", dropna=True):
        ...
        days = b["Day_of_Year"].to_numpy(dtype=dtype, copy=True)
        if days.size == 0:
            continue
        out[int(y)] = {
            "days":   days,
            "temp":   b["Average Temp"].to_numpy(dtype=dtype, copy=True),
            "ppfd":   b["PPFD (µmol/m²/s)"].to_numpy(dtype=dtype, copy=True),
            "hum":    b["Average Humidity (%)"].to_numpy(dtype=dtype, copy=True),
            "rain":   b["rain"].to_numpy(dtype=dtype, copy=True),
            "pe":     b["pe"].to_numpy(dtype=dtype, copy=True),
            "smd_wd": b["smd_wd"].to_numpy(dtype=dtype, copy=True),
            "smd_md": b["smd_md"].to_numpy(dtype=dtype, copy=True),
            "smd_pd": b["smd_pd"].to_numpy(dtype=dtype, copy=True),
        }
    return out
...
def strawberry_growth_scalar(G_max, temperature, PPFD, humidity, soil_moisture_mm,
    ...
    # gaussian-like effects (normalized near 1 at optima)
    def _scaled(x, mu, sigma=0.3):
        denom = 1.0 / (np.sqrt(2.0 * np.pi) * sigma)
        return np.exp(-0.5*((x - mu)/sigma)**2) / denom
    ...
    # seasonal logistic derivative uses planting_duration (matches baseline)
...
if HAS_NUMBA:
    ...
    def _simulate_one_year_kernel(
        ...
        G_max, weather_sigma,
        planting_duration,     # match scalar timing
        seed                   # per-(year,sim) seed
    ):
        ...
        for i in range(n):
            ...
            temp_effect = np.exp(-0.5*((Tn - mu_T)/sigma)**2) / denom
            hum_effect  = np.exp(-0.5*((Hn - mu_H)/sigma)**2) / denom
            sm_effect   = np.exp(-0.5*((Mn - mu_M)/sigma)**2) / denom
            ...
            # fertilizer effect from same RNG
...
with st.sidebar:
    ...
    use_float32 = st.checkbox("Use float32 arrays (faster, smaller)", value=False)  # default False for parity
    want_numba  = st.checkbox("Use Numba acceleration", value=True, disabled=not HAS_NUMBA)
...
if load_btn:
    if not xlsx_path.exists():
        ...
        ss.year_arrays_key = (str(xlsx_path.resolve()), bool(use_float32))
...
def _get_year_arrays():
    if "year_arrays" not in ss:
        ss.year_arrays = {}
    if "year_arrays_key" not in ss:
        ss.year_arrays_key = None
    if not ss.loaded or ss.df is None:
        return {}
    key = (str(Path(xlsx_path).resolve()), bool(use_float32))
    if ss.year_arrays_key != key or not ss.year_arrays:
        ss.year_arrays = preprocess_year_arrays(ss.df, use_float32)
        ss.year_arrays_key = key
    return ss.year_arrays

# --------------------------- Main simulation ---------------------------
def run_simulations_fast(df, year_from, year_to, want_numba, use_float32, show_progress,
    ...
    # Seed host RNGs for any incidental use; per-sim seeds will be derived below
    ...
    for i, year in enumerate(years, start=1):
        ...
        if not arr:
            ...
            if show_progress: progress.progress(i/len(years))
            continue
        ...
        for s in range(nsims):
            ...
            if want_numba and HAS_NUMBA:
                total_per_m2 = _simulate_one_year_kernel(
                    ...
                    float(G_max), float(weather_variability),
                    int(planting_duration),
                    int(seed_s)
                )
                sims[s] = total_per_m2 * area

                # Small number of stored daily series (Python pass) to keep things fast
                if per_sim_daily_year is not None and len(per_sim_daily_year) < int(max_daily_store):
                    rng = np.random.default_rng(int(seed_s) + 12345)  # separate stream for storage pass
                    current_sm = 0.5 * capacity
                    daily = {}
                    for idx in range(arr["days"].shape[0]):
                        doy = int(arr["days"][idx])
                        if p_doy <= doy <= h_doy:
                            w = float(weather_variability)
                ...
                # Pure Python path with same seed discipline
                ...
                for idx in range(arr["days"].shape[0]):
                    ...
                    if p_doy <= doy <= h_doy:
                        ...
                        current_sm = soil_step(current_sm, capacity, planting_depth, rn, ev, runoff_coeff, drainage_rate)
                        fe = rng.uniform(1.0, 1.2)  # fertilizer draw from same stream
                        g = strawberry_growth_scalar(G_max, t, L, h, current_sm, capacity, doy, p_doy, planting_duration, fe)
                        daily[doy] = daily.get(doy, 0.0) + g
    ...
    if not ss.loaded or ss.df is None:
        ...
        with st.spinner("Simulating…"):
            results_df, per_sim_yields, per_sim_daily, planting_windows = run_simulations_fast(
                ...
                want_numba=HAS_NUMBA and want_numba,
                use_float32=use_float32, show_progress=show_progress,
                ...
                num_simulations=num_simulations, weather_variability=weather_variability, random_seed=int(random_seed),
...
if ss.results_df is not None:
    ...
    year_choice = st.selectbox("Select a year", options=list(ss.results_df["Year"]), index=0)
    if year_choice in ss.per_sim_yields:
        yvals = ss.per_sim_yields[year_choice]
        sim_ids = list(range(1, len(yvals) + 1))
        fig2, ax2 = plt.subplots(figsize=(10, 5))
        ax2.plot(sim_ids, yvals, marker="o", linewidth=1)
        ax2.set_xlabel("Simulation #"); ax2.set_ylabel("Total yield (kg)")
        ...
        st.download_button("Download per-simulation yields (CSV)",
    ...
    st.download_button("Download summary (CSV)",
                       ...
                       file_name="yearly_yield_summary.csv", mime="text/csv")
    buf = BytesIO()
    with pd.ExcelWriter(buf, engine="xlsxwriter") as writer:
        ss.results_df.to_excel(writer, index=False, sheet_name="Yearly Yields")
    st.download_button("Download summary (Excel)",
                       data=buf.getvalue(),
                       file_name="yearly_yield_summary.xlsx",
                       mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

    # Plot 3: Daily growth (if stored)
    ...
    if ss.per_sim_daily:
        ...
        if years_with_series:
            ...
            if st.checkbox("Show cumulative growth overlay"):
...
# --------------------------- Notes ---------------------------
with st.expander("Why these design choices?", expanded=False):
    st.markdown(
        "- **Deterministic RNG:** per-(year,sim) seeds make results reproducible across Python and Numba.\n"
        "- **Sorted arrays:** soil moisture is stateful; chronological order is essential for parity.\n"
        "- **Season timing parity:** kernel uses `planting_duration` like the scalar function.\n"
        "- **Float64 default:** matches baseline numerics; enable float32 if you prefer speed over exact parity.\n"
        "- **Daily series throttling:** stores only a few per year so the main loop stays fast."
    )
